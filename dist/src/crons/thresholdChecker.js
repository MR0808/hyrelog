 ...
 */
export const runThresholdChecker = async () => {
    const now = new Date();
    ...
    for (const meter of activeMeters) {
        ...
        if (currentPercent >= threshold.softLimitPercent) {
            const result = checkThreshold(currentPercent, threshold.softLimitPercent, ThresholdType.PERCENTAGE);
            if (result.triggered) {
        ...
        if (currentPercent >= threshold.hardLimitPercent) {
            const result = checkThreshold(currentPercent, threshold.hardLimitPercent, ThresholdType.PERCENTAGE);
            if (result.triggered) {
                const existingAlert = await prisma.thresholdAlert.findFirst({
                    where: {
                        companyId: meter.companyId,
