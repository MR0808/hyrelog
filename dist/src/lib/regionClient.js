 ...
 */
let regionDataStoreCache = null;
/**
 * Loads region data store records from the database.
 */
async function loadRegionDataStore() {
 ...
 */
export async function getPrismaForRegion(region) {
 ...
 */
export async function getRegionForCompany(companyId) {
 ...
 */
export async function getPrismaForCompany(companyId) {
 ...
 */
export async function getColdStorageClientForRegion(region) {
    // Load region data store if not cached
    if (!regionDataStoreCache) {
        await loadRegionDataStore();
    }
 ...
 */
export async function closeAllRegionClients() {
    await Promise.all(Array.from(regionClients.values()).map((client) => client.$disconnect()));
    regionClients.clear();
}
//# sourceMappingURL=regionClient.js.map