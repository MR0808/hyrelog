...
exports.default = companyWebhookDeliveryRoutes;
const DEFAULT_LIMIT = 50;
const MAX_LIMIT = 200;
async function companyWebhookDeliveryRoutes(fastify) {
    /**
     * ---------------------------------------------------------------------
     ...
     * ---------------------------------------------------------------------
     */
    fastify.get('/v1/company/webhook/deliveries', {
        config: {
        ...
        const where = { companyId: auth.companyId };
        if (status)
        ...
        const nextCursor = deliveries.length === take
     ...
     * ---------------------------------------------------------------------
     */
    fastify.get('/v1/company/webhook/deliveries/:id', {
        config: {
            rateLimit: { max: 30, timeWindow: '1 minute' }
        }
    }, async (request, reply) => {
