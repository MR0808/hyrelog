...
import { ApiKeyType, BillingCycle, BillingMeterType, DataRegion, Region, PrismaClient, WebhookDeliveryStatus, ThresholdType, GdprRequestType, GdprRequestStatus, InternalUserRole } from '@prisma/client';
...
const keyPrefix = (raw) => raw.slice(0, 12);
...
async function main() {
    ...
    const hashedGlobalTechOwnerPassword = await hashPassword(globalTechOwnerPassword);
    ...
    const hashedGlobalTechAdminPassword = await hashPassword(globalTechAdminPassword);
    ...
    const hashedGlobalTechMemberPassword = await hashPassword(globalTechMemberPassword);
    ...
    const meters = await Promise.all(companies.map((company) => prisma.billingMeter.create({
        data: {
    ...
    const workspaceKeys = await Promise.all(workspaceKeyPairs.map(({ workspace, rawKey }) => prisma.apiKey.create({
        data: {
            companyId: workspace.companyId,
            workspaceId: workspace.id,
    ...
    const workspaceHashMap = new Map();
    ...
    const categories = ['auth', 'billing', 'project', 'user'];
    const events = [];
    ...
    for (let i = 0; i < 50; i += 1) {
        ...
        const payload = { message: `Event #${i + 1}` };
        const changes = action === 'user.updated'
            ? [
                {
                    field: 'name',
                    old: `Old Name ${i}`,
        ...
        const projectId = i % 3 === 0 ? projects[i % projects.length]?.id : null;
        ...
        const event = await prisma.auditEvent.create({
            data: {
                ...
                prevHash,
                traceId: `trace-${i}`,
                dataRegion: workspace.companyId === acmeCorp.id
                    ? DataRegion.US
                    : DataRegion.EU,
                createdAt,
                archived: false,
        ...
        if (i % 5 === 0 && workspace.companyId === acmeCorp.id) {
            const matchingWebhooks = webhooks.filter((w) => w.companyId === workspace.companyId &&
                (w.workspaceId === null ||
    ...
    for (let i = 0; i < 30; i += 1) {
        ...
        if (workspace.companyId !== acmeCorp.id)
            continue;
        const prevHash = workspaceHashMap.get(workspace.id) ?? null;
    ...
    console.log(`✅ Created ${oldEvents.length} old events marked for archival\n`);
    ...
    console.log(`✅ Created ${archivedEvents.length} already archived events\n`);
    // ============================================
    ...
    const dataPlatformWorkspace = allWorkspaces.find((w) => w.slug === 'data-platform');
    ...
    const totalEvents = events.length + oldEvents.length + archivedEvents.length;
    ...
    console.log(`✅ Created ${events.length} recent events, ${oldEvents.length} old events (ready for archival), ${archivedEvents.length} archived events\n`);
    // ============================================
    ...
    console.log(`   Total Events: ${events.length + oldEvents.length + archivedEvents.length}`);
    ...
    console.table([
        ...
        ...workspaceKeys.map((key, idx) => {
            ...
            const company = companies.find((c) => c.id === pair.workspace.companyId);
            return {
                name: key.name,
                type: 'workspace',
    ...
    console.table(templates.map((t) => ({
        ...
        company: t.companyId
            ...
            : 'Global',
        assigned: t.id === templates[1].id ? acmeWorkspaces[0].slug : 'none'
    })));
    ...
    console.log('   You can sign up with these emails and use the generated passwords,');
    console.log('   or create accounts programmatically through Better-Auth\'s API.');
    console.log('   Internal admin user (mark@hyrelog.com) password is already set.\n');
    console.log('\n💡 Next Steps:');
    console.log('   1. Test API endpoints with the API keys above');
    console.log('   2. Login to dashboard with the user credentials above');
    console.log('   3. Test internal admin portal at /internal/login with mark@hyrelog.com');
    console.log('   4. Run archival job: npm run worker (or wait for cron at 4 AM)');
    console.log('   5. Test archive export: GET /v1/key/company/export-archive.json');
    ...
    console.log('\n📦 Archival Testing:');
    console.log(`   - ${oldEvents.length} events are marked as archival candidates`);
    console.log(`   - These events are 400-450 days old (older than 365 day retention)`);
