...
async function main() {
    ...
    const workspaces = await Promise.all(["core-app", "data-platform"].map((slug, index) => prisma.workspace.create({
        data: {
    ...
    const actions = ["user.login", "user.logout", "invoice.created", "invoice.paid"];
    const categories = ["auth", "billing", "project"];
