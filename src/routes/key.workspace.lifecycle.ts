...
export const keyWorkspaceLifecycleRoutes: FastifyPluginAsync = async (app) => {
    /**
    ...
    app.post('/v1/key/workspace/create', async (request, reply) => {
        ...
        const { name, readOnly, labels, ipAllowlist, expiresAt } =
            createKeyBodySchema.parse(request.body);
    ...
    app.get('/v1/key/workspace/usage', async (request, reply) => {
        ...
        const healthScore = Math.max(
            0,
            Math.min(100, Math.round((1 - errorRate) * 100))
        );
