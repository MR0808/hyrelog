...
export const keyWorkspaceEventsRoutes: FastifyPluginAsync = async (app) => {
    app.get('/v1/key/workspace/events', async (request) => {
        ...
        const fromDate = filters.from
            ? new Date(
                  Math.max(filters.from.getTime(), retention.start.getTime())
              )
            : retention.start;
        ...
        const { events, total } = await queryWorkspaceEvents(
            workspace.id,
            queryFilters
        );
    ...
    app.post('/v1/key/workspace/events', async (request, reply) => {
        ...
        const event = await ingestEventToRegion(ctx.company.id, workspace.id, {
            ...
            createdAt
        } as Parameters<typeof ingestEventToRegion>[2] & { createdAt: Date });
