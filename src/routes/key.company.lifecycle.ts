...
export const keyCompanyLifecycleRoutes: FastifyPluginAsync = async (app) => {
  /**
  ...
  app.post("/v1/key/company/create", async (request, reply) => {
    ...
    const { name, readOnly, labels, ipAllowlist, expiresAt } = createKeyBodySchema.parse(
      request.body,
    );
  ...
  app.get("/v1/key/company/usage", async (request, reply) => {
    ...
    const avgLatency =
      logs.length > 0 ? logs.reduce((sum, log) => sum + log.latencyMs, 0) / logs.length : 0;
    ...
    const endpointStats = logs.reduce(
      ...
      {} as Record<string, { count: number; errors: number; avgLatency: number }>,
    );
